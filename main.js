...
if (urlParams.has("room")) {
	document.getElementById("container-2-Comm").style.visibility = "visible";
}
if (urlParams.has("director")) {
	document.getElementById("DirectorComm").style.visibility = "visible";
}

const queryString = window.location.search;
console.log(queryString);
var urlParams = new URLSearchParams(queryString);
var group = urlParams.get("room");
var sendMSG = document.querySelector("#sendmessage");
...
if (group == null) {
	group = urlParams.get("director");
	sendMSG = document.querySelector("#sendmessagedir");
	MSG = document.querySelector("#messagesdir");
	MSGBox = document.querySelector("#messageBoxdir");
	Playback = document.querySelector("#Playback");
}
console.log(group);
var name = "";

var ws = new WebSocket('wss://node.presentationman.com:8080');
...
function showMessage(message) {
	...
	if (urlParams.has("room")) {
		...
		if (message.msg == "Forward" || message.msg == "Rewind") {
			Playback.textContent += `\n${resname} Pressed ${message.msg}`;
		} else {
			messagesdir.textContent += `\n${resname} - ${message.msg}`;
			messagesdir.scrollTop = messagesdir.scrollHeight;
			messageBoxdir.value = "";
		}
	}
}

//For arrow function JSON.parse here, then filter by if(ms.data.arrow) if not then push to showMessage... if it does then go to the seperate arrow display function
ws.onmessage = (ms) => {
	console.log(ms.data);
	showMessage(ms.data);
}
...
function broadcast(msg, room) {
	ws.send(JSON.stringify({ room: room, msg: msg, name: name }));
}
function join(room) {
...
MSGBox.addEventListener("keyup", (event) => {
	if (event.keyCode === 13) {
		...
		if (name == "") {
			name = prompt("Please enter a name to use the Chat.", "John Smith");
		}

		//Prevents empty messages from being sent over the websocket.
		if (urlParams.has("room")) {
			if (messageBox.value != "") {
				broadcast(messageBox.value, group);
			} else {
				console.log("User tried to send an empty message");
			}
		} else {
			if (messageBoxdir.value != "") {
				broadcast(messageBoxdir.value, group);
			} else {
				console.log("User tried to send an empty message");
			}
		}
...
Forward.onclick = () => {
	...
	if (name == "") {
		name = prompt("Please enter a name to use Playback Functions.", "John Smith");
	}

	if (urlParams.has("room")) {
		broadcast("Forward", group);
	}
}
